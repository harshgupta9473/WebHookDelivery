...
func UpdateWebhookStatusAndDelivery(webhookID uuid.UUID, status string, delivered bool, updatedAt time.Time) error {
	query := `UPDATE webhooks 
	          SET status = $1, delivered = $2, updated_at = $3 
	          WHERE id = $4`

	_, err := db.DB.Exec(query, status, delivered, updatedAt, webhookID)
	if err != nil {
